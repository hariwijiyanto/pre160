...
def init_secp256k1_constants(mod):
    ...
    const_p_gpu = mod.get_global("const_p")[0]; cuda.memcpy_htod(const_p_gpu, p_data)
    n_data = np.array([0xD0364141,0xBFD25E8C,0xAF48A03B,0xBAAEDCE6,0xFFFFFFFE,0xFFFFFFFF,0xFFFFFFFF,0xFFFFFFFF],dtype=np.uint32)
    const_n_gpu = mod.get_global("const_n")[0]; cuda.memcpy_htod(const_n_gpu, n_data)
...
def main():
    ...
    parser.add_argument('--keys-per-launch', type=int, default=1<<20)
    parser.add_argument('--kernel-file', default='kernel160.cu')
    ...
    try:
        ...
        print("kernel file not found"); sys.exit(1)
